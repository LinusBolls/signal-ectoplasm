...
console.log('DEFAULT_DB_SQLITE_PATH', DEFAULT_DB_SQLITE_PATH);
...
export default class SignalHistoryClient {
  ...
  private async _connect() {
    ...
    if (!encryptedKey) {
      throw new Error(
        `[signal-history-client] FATAL error: property "encryptedKey" not found in "${
          this.configJsonPath
    ...
    console.log('before decrypting key:', encryptedKey);
    const decryptedKey = safeStorage.decryptString(encrypted);
    console.log('after decrypting key');

    // use SQLCipher v4, the protocol Signal uses to encrypt db.sqlite
    ...
    this.db!.run(`PRAGMA key = "x'${decryptedKey}'"`);
  ...
  async _dbAll<T extends unknown>(
    ...
    const res = await new Promise<[null, T[]] | [unknown, []]>(
      (resolve, reject) => {
        ...
        const tryToExecuteQuery = async () => {
          if (!this.isConnected) {
            ...
            throw new Error(
              "if you see this error, i'm afraid SignalHistoryClient is fatally broken"
            );
          }
          ...
          this.db!.all(query, (err: Error, rows: T[]) => {
            if (err) {
              ...
              if (!sqliteErr.mightBeSolvedByRetry) {
                console.error(
                  `[signal-history-client] FATAL query failed: "${sqliteErr.id}"`
                );
              ...
              if (currentConnectionRetries > MAX_CONNECTION_RETRIES) {
                console.error(
                  '[signal-history-client] FATAL query failed after exceeding maximum amount of retries'
                );

                resolve([
                  'Exceeded maximum amount of retry attempts trying to connect to database',
                  [],
                ]);
              ...
              setTimeout(
                tryToExecuteQuery,
                WAIT_TIME_UNTIL_CONNECTION_RETRY_MS
              );

              return;
            }
            if (currentConnectionRetries > 0)
              console.info(
                `[signal-history-client] query succeeded after ${currentConnectionRetries} retries`
              );
